...
def run_tests(live_utxo_manager=None, live_mempool=None, test_id=-1):
    ...
    print(f"\n=== EXECUTING TEST CASE: {test_id if test_id != -1 else 'ALL'} ===\n")

    # 1. Test 1
    if test_id in [1, -1]:
        print("Test 1: Alice sends 10 BTC to Bob (0.001 Fee)")
        print("Action: Alice uses genesis:0 (50 BTC) to pay Bob 10 BTC.")
    ...
    if test_id in [2, -1]:
        ...
        print("Action: David (10 BTC) and Eve (5 BTC) combine UTXOs to send 14.99 BTC to Bob.")
    ...
    if test_id in [3, -1]:
        ...
        print("Action: Bob tries to use the SAME UTXO (genesis:1) twice in one TX.")
    ...
    if test_id in [4, -1]:
        ...
        print("Action: Alice tries to send 5 BTC to Charlie using genesis:0 (already used in TX1).")
    ...
    if test_id in [5, -1]:
        ...
        print("Action: Bob tries to spend 35 BTC using a 30 BTC UTXO.")
    ...
    if test_id in [6, -1]:
        ...
        print("Action: Charlie tries to create a negative output (-5.0 BTC).")
    ...
    if test_id in [7, -1]:
        ...
        print("Action: Bob sends exactly 30 BTC to Charlie using his 30 BTC input.")
    ...
    if test_id in [8, -1]:
        ...
        print("Action: Simulation of conflicting transactions.")
        print("Result: Success - Handled by mempool spent_utxos logic (See Test 4).\n")

    # 9. Test 9
    if test_id in [9, -1]:
        print("Test 9: Complete Mining Flow")
        print(f"Action: Processing {len(mempool.transactions)} transactions into a block.")
    ...
    if test_id in [10, -1]:
        ...
        print("Action: Attempting to spend a UTXO not yet in the blockchain.")
        print("Design Decision: Simulator REJECTS unconfirmed spends.\n")

    if test_id == -1:
        print("=== FINAL BALANCES AFTER ALL TESTS ===")
...
if __name__ == "__main__":
    ...
    print("Enter Test Case Number (1-10) or -1 for all:")
    choice = input("Choice: ")
    try:
        run_tests(test_id=int(choice))
    except ValueError:
        print("Invalid input.")